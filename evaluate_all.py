...
for cat in ['slow', 'medium', 'fast']:
    ...
    temp = [ os.path.join(cat_dir, name) for name in os.listdir(cat_dir) if os.path.isdir(os.path.join(cat_dir, name)) ] #list only directories
    dirs += temp
...
print(results)
results = results.drop(20) # usunięcie odstającego punktu dla szybkiego chodu
